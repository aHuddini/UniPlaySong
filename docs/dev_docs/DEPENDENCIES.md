...
- **Game Music Emu (gme.dll)**: **LGPL v2.1+** — compatible via dynamic linking. UniPlaySong links via P/Invoke; `gme.dll` is a separate file in the `.pext`, making it user-replaceable. Our build uses `GME_YM2612_EMU=Nuked` (LGPL-safe); the MAME YM2612 core (GPL v2+) is explicitly NOT used. Source pin and reproduction in [`features/CHIPTUNE_GME_DLL_BUILD.md`](features/CHIPTUNE_GME_DLL_BUILD.md).
