...
| [DOCS_REORGANIZATION_TODO.md](roadmaps/DOCS_REORGANIZATION_TODO.md) | Backlog for reorganising this folder — `features/` split, moving the theme guide out of dev_docs, archiving superseded plans, gitignoring proposals. Work that was done on a branch and deliberately dropped rather than merged; redo the intent, re-check every path first. |
