...
| **System-Audio Visualizer** | Drive the desktop spectrum visualizer from whole-device WASAPI loopback (`WasapiLoopbackCapture`, already in the NAudio backend) so it reacts to *whatever the speakers are playing*, including external players like Spotify. NOT Spotify-targeted — captures the full device mix (game audio + UPS + Spotify together) and is framed honestly as a "system audio visualizer." Workaround for the Spotify-control feature's limitation that UPS can't visualize Spotify's DRM-locked audio. Keep transient (no storage), no Spotify API/branding, to stay in the lowest ToS-risk corner. | Medium | Medium |
