...
Cross-ref: [SPOTIFY_INTEGRATION.md](../dev_docs/features/SPOTIFY_INTEGRATION.md) (SMTC transport control), [NAUDIO_PIPELINE.md](../dev_docs/features/NAUDIO_PIPELINE.md) (the effect chain we'd feed).
...
2. **Managed bridge** — P/Invoke the shim, marshal the PCM callback into a `WaveProvider`/`ISampleProvider` that feeds the existing NAudio effect chain ([NAUDIO_PIPELINE.md](../dev_docs/features/NAUDIO_PIPELINE.md)).
