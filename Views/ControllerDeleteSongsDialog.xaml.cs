...
namespace UniPlaySong.Views
{
    ...
    public partial class ControllerDeleteSongsDialog : UserControl
    {
        ...
        private static uint XInputGetState(uint dwUserIndex, ref XINPUT_STATE pState)
        {
            try { return XInputGetState14(dwUserIndex, ref pState); }
            catch { }
            try { return XInputGetState13(dwUserIndex, ref pState); }
            catch { }
            try { return XInputGetState910(dwUserIndex, ref pState); }
            catch { return 1; } // ERROR_DEVICE_NOT_CONNECTED
        }
        ...
        public void InitializeForGame(Game game, IPlayniteAPI playniteApi, GameMusicFileService fileService, IMusicPlaybackService playbackService)
        {
            try
            {
                ...
                Logger.Debug($"Initialized delete songs dialog for game: {game?.Name}");
        ...
        private void PopulateFilesList()
        {
            ...
            foreach (var filePath in _musicFiles)
            {
                ...
                {
                    Logger.Debug(ex, $"Error getting file size for {filePath}");
                }
        ...
        private void StartControllerMonitoring()
        {
            ...
            Logger.Debug("Started controller monitoring for delete songs dialog");
        }
        ...
        private void StopControllerMonitoring()
        {
            ...
            Logger.Debug("Stopped controller monitoring for delete songs dialog");
        }
        ...
        private async Task CheckButtonPresses(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    uint result = XInputGetState(0, ref state); // Check controller 0
        ...
        /// </summary>
        private void HandleControllerInput(ushort pressedButtons, XINPUT_GAMEPAD gamepad)
        {
            try
            {
                ...
                if ((pressedButtons & XINPUT_GAMEPAD_A) != 0)
                {
                ...
                // B button - Cancel
                if ((pressedButtons & XINPUT_GAMEPAD_B) != 0)
                {
                ...
                // X/Y buttons - Preview
                if ((pressedButtons & (XINPUT_GAMEPAD_X | XINPUT_GAMEPAD_Y)) != 0)
                {
                    PreviewSelectedFile();
                    return;
                }
                ...
                // D-Pad navigation
                if ((pressedButtons & XINPUT_GAMEPAD_DPAD_UP) != 0)
                {
                    NavigateList(-1);
                }
                else if ((pressedButtons & XINPUT_GAMEPAD_DPAD_DOWN) != 0)
                {
                    NavigateList(1);
                }

                // Shoulder buttons - Page navigation
                if ((pressedButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) != 0)
                {
                    NavigateList(-5); // Page up
                }
                else if ((pressedButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) != 0)
                {
        ...
        private void ConfirmAndDeleteSong(string filePath)
        {
            try
            {
                ...
                Logger.Debug($"Showing confirmation dialog for: {fileName}");
                ...
                Logger.Debug($"Confirmation result: {result}");
        ...
        private void StopAllMusicPlayback(string fileName)
        {
            try
            {
                Logger.Debug($"Stopping all music playback before deleting: {fileName}");
                ...
                if (_playbackService != null)
                {
                    if (_playbackService.IsPlaying)
                    {
                        ...
                        Logger.Debug("Stopped game music playback for file deletion");
        ...
        private void DeleteSongFile(string filePath, bool isPrimarySong)
        {
            try
            {
                ...
                {
                    StopCurrentPreview();
                    Logger.Debug("Stopped preview for file being deleted");
                }
                ...
                Logger.Debug($"Removed from music files list: {fileName}");
            ...
            finally
            {
                ...
                ResetDeletionState();
                Logger.Debug("Deletion process completed, state reset");
            }
        }
