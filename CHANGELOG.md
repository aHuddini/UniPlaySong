...
- **Extension prevents Windows from automatic system suspension after UPS has played audio at least once** (issue #81, reported by @darklinkpower). Root cause is architectural: as of v1.3.3 UPS uses a persistent audio device (`WaveOutEvent` for NAudio, `Mix_OpenAudio` for SDL2) that stays open for the lifetime of the plugin. This was a deliberate trade-off — eliminates ~70ms UI freeze and audible click on every game-switch song change — but means Windows considers an active audio session running at all times (visible as Playnite under `[DRIVER]` in `powercfg /requests`), which blocks system sleep. Users on desktop builds with autosuspend configured will see this as Windows never going to sleep while Playnite is running. **Workaround for now**: fully exit Playnite when stepping away (Windows can suspend normally with Playnite not running); restarting Playnite later reinitializes the audio device fresh. A v1.5.x fix is being investigated — likely a combination of an explicit `SetThreadExecutionState(ES_CONTINUOUS)` opt-out call on pause (low-risk, removes UPS's explicit "keep awake" hint) and a longer-window idle-teardown timer (~4-5 min) that releases the audio device when UPS has been idle long enough that the user is clearly away from the machine.
