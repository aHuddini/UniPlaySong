...
- **A sixth achievement rarity, Hidden, for secret unlocks**, sitting between Ultra-Rare and Capstone. Fired by Playnite Achievements as `playnite://uniplaysong/playniteachievements/hidden` — note the segment is `hidden`, not `hiddenachievement`, because that is the command name PA's developer chose; the other five keep the `{rarity}achievement` pattern. Themes supply it as `audio/Achievements/hidden.wav` (`.mp3`/`.ogg`/`.flac` also resolve), and the theme status line counts N/6 rather than N/5. `src/Services/JingleService.cs`, `src/Services/ExternalControlService.cs`, `src/Common/PlayniteThemeHelper.cs`, `src/Services/BundledJingleService.cs`, `src/UniPlaySongSettings.cs`, `src/UniPlaySongSettingsViewModel.cs`, `src/UniPlaySongSettingsView.xaml`, `src/UniPlaySongSettingsView.xaml.cs`.
