...
- **Two new bundled-ambient default-music tracks from Mike Aniki ([Aniki ReMake](https://github.com/Mike-Aniki/Aniki-ReMake) theme developer)** — `ANIKI REMAKE - Hub OST.mp3` and `ANIKI REMAKE - Login OST.mp3` ship in the `.pext` as Bundled Ambient presets, included with explicit permission from the composer. New entries in `src/DefaultMusic/presets.json`. The .mp3 files themselves are excluded from the source repo via `.gitignore` (packaging script copies them into the .pext from the developer's local copy). Credit added to `NOTICES.txt`.
- **Theme integration — fullscreen-bindable settings.** `UPS_MusicControl` now exposes four of the most-toggled UPS settings as two-way bindable WPF properties so Fullscreen theme authors can wire CheckBox / ToggleButton state directly to UPS settings: `EnableGameMusic`, `EnableDefaultMusic`, `RadioModeEnabled`, `PlayOnlyOnGameSelect`. Standard pattern: place a `<UPS:MusicControl x:Name="upsAudio" Visibility="Collapsed"/>` somewhere in the theme's XAML and bind via `IsChecked="{Binding ElementName=upsAudio, Path=EnableGameMusic, Mode=TwoWay}"`. Setter assignments flow through the existing `UniPlaySongSettings` PropertyChanged → settings-service → playback-coordinator pipeline, so a theme toggle has the same effect as flipping the corresponding setting in the desktop UPS settings dialog. The `EnableGameMusic` / `EnableDefaultMusic` split lets themes offer independent control over the two audio layers (game-specific music vs. fallback ambient) — toggling only `EnableGameMusic` off leaves default music playing as a fallback per UPS's longstanding behavior; toggle both off for full silence. Designed in collaboration with Mike Aniki for the Aniki ReMake theme's quick-audio-settings menu.
- **`Enable Music` toggle** added to the existing Fullscreen quick-settings menu (Menu → Extensions → UniPlaySong). All three theme-integration priority settings (`Enable Music`, `Radio Mode`, `Play Only on Game Select`) are now reachable from both the standard Playnite Fullscreen Extensions menu AND via theme-bound widgets via `UPS_MusicControl`. Radio Mode and Play Only on Game Select were already in the menu — Enable Music was the gap.
