...
- Added `VizBarCount` (Live Effects -> Visualizers, 1-12, default 12) — the spectrum visualizer's four tuning tables are calibrated for 12 bars, so `BuildBars` resamples them; the mapping is the identity at 12
- Added `ShowCalmDownButton` (Media Controls, default off) — a moon button on the Desktop top panel that toggles Calm Down, matching the Fullscreen quick menu
- Added a Live Effects -> Calm Down page — `SelectedCalmDownPreset` plus sliders for cutoff, volume and transition time, which had been config-file-only and unclamped until now
- Added `CalmDownOnIdle` / `CalmDownIdleTimeoutMinutes` (default off, 10 minutes) — engages Calm Down after a stretch with no input, via a runtime-only flag so it never writes the user's own toggle
- Graduated `AutoLaunchSpotifyOnStartup` and `SpotifyExePath` out of Experimental to General -> Miscellaneous, moving their reset group with them
- Graduated `NaudioFadeInCurve` / `NaudioFadeOutCurve` out of Experimental to Live Effects -> Fade Transitions, in a collapsed Advanced section

### Changed

- Made `JingleVolume` an independent level rather than a share of `MusicVolume`, which had capped how loud jingles could get; the default is rebased 100 -> 50 so a fresh install sounds unchanged
- Moved Calm Down ahead of Visualizers in the Live Effects rail and tidied its page
- Highlighted every "restart required" note the same way — five stated it in plain body text, where the one thing the reader had to notice looked like the rest of the sentence

### Fixed

- Fixed UniPlaySong halting Playnite's own Fullscreen background music seconds after startup (Solaris [#137](https://github.com/And360red/Solaris/issues/137)) — `Mix_HaltMusic` is process-global and the jingle device prewarm reached it through `Close()`; `Stop()` now halts only when this instance owns the stream. Reported by **Charade28**, diagnosed and bisected to 1.7.2 by **And360red**
- Fixed the settings Preview buttons playing at `MusicVolume` instead of the jingle level, which only diverged once the two were decoupled this release
- Fixed saving settings reverting anything changed outside the dialog — the cached view model held the settings object loaded at startup, so `EndEdit` wrote it back over newer ones; `BeginEdit` now adopts `SettingsService.Current`
- Fixed Random Game Picker music playing over the library's own ([#95](https://github.com/aHuddini/UniPlaySong/issues/95)) — its guard sat after `ShouldPlayMusic`, leaving earlier branches free to act on playback mid-picker
- Fixed the Random Game Picker ignoring Where Music Plays ([#95](https://github.com/aHuddini/UniPlaySong/issues/95)) — the mode rule moved to `UniPlaySongSettings.AllowsMusicInMode`, shared with the coordinator
- Fixed the Music Dashboard toggle not raising the restart prompt it needs ([#95](https://github.com/aHuddini/UniPlaySong/issues/95)), and removed the live visibility handler that could never have worked

### Documentation

- Brought the developer documentation up to date with the current architecture, including the settings rail structure, the theme integration guide, and the two undocumented URI sound events
