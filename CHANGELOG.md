...
- **`NaudioFadeInCurve` / `NaudioFadeOutCurve` graduated out of Experimental.** They move from the Experimental page to Live Effects -> Fade Transitions, in an `Expander` with `IsExpanded="False"` titled Advanced — the shape of a fade is a finer point than its length, so the page still opens on the two settings people actually change. Reset group moves with them (Advanced -> Live Effects) to match the page.
  - The hint lost its "(NAudio pipeline)" aside; the condition now reads "only applies with Live Effects or the Visualizer switched on", which is the same fact in terms the reader can act on.
- **`AutoLaunchSpotifyOnStartup` graduated out of Experimental.** It and `SpotifyExePath` move from the Experimental page to General -> Miscellaneous, and their reset group moves with them (Advanced -> General) so the page and the button that clears it agree. Default is unchanged at off — launching another application on startup is not something to opt anyone into. Behaviour is untouched; this is a move.
- **Every "restart required" note in the settings uses the same highlight.** Five hints stated it in plain body text — the Desktop sidebar button, the Music Dashboard, media-key control, Spotify-as-fallback and Spotify-as-radio-source — so the one thing the reader needed to notice looked like the rest of the sentence. They now split the notice into its own `<Run>` with `Foreground="{StaticResource UpsWarn}"` and `FontWeight="SemiBold"`, matching the Media Controls page. Style, margins and wrapping are unchanged; only the notice sentence is repainted.
  - `StaticResource` failures are runtime, not build, errors, so a green build proves nothing here. Verified by rendering each page headlessly (`Assembly.LoadFrom` → `CreateInstance` → Measure/Arrange) and walking the visual tree to assert every restart `Run` resolves to `#C8A24B`.
- **Calm Down sits before Visualizers in the Live Effects rail, and its page was tidied.** "Enable Calm Down Mode" on the regular toggle style rather than the amber one, the preset dropdown directly under the toggle it belongs to with no section header of its own, and a description that says the setting is shared by Desktop and Fullscreen (reachable live from Extensions -> [UPS] Calm Down Mode) instead of naming the player backend.
- **`CalmDownOnIdle` / `CalmDownIdleTimeoutMinutes` (Live Effects -> Calm Down, default off, 10 minutes).** Engages Calm Down after a stretch with no keyboard or mouse input and releases it on the next input, riding the existing `OnIdlePollTick` alongside Pause on Idle and Lower Volume on Idle (`GetLastInputInfo`, so gamepad input does not count — same limitation as the other two).
  - Idle drives a separate runtime-only `CalmDownIdleActive`, which `CalmDownProcessor` ORs with `CalmDownModeEnabled`; it never writes the user's own setting. Writing it would persist to disk, tick the settings checkbox, and — the real hazard — releasing on input would switch off a Calm Down the user had turned on themselves. Both routes ramp identically, so idle gets the same S-curve and the same preset.
  - `[JsonIgnore]` on the flag keeps a transient state out of the settings file and out of the JSON clone a save round-trips through. Because that clone drops it, the poll re-asserts the flag every tick rather than only on the transition — otherwise an unrelated save mid-idle would quietly lift Calm Down until the next keypress. The write is guarded on a value change, so it does not raise `PropertyChanged` 1-2x/second.
  - Enabling the setting forces the NAudio backend up front, the same way `CalmDownModeEnabled` does. Deciding it at idle time would mean swapping the player mid-idle, which restarts the song.
  - Filed in `NeverReset` rather than a reset group: it is live state, false at every launch, with nothing for a reset to restore. `SettingsResetCoverageTests` caught the omission by name.
- **Live Effects -> Calm Down page.** `SelectedCalmDownPreset` (Custom, Default, Subtle, Warm, Muffled, Distant, Whisper) plus sliders for `CalmDownLowPassCutoffHz`, `CalmDownVolumeMultiplier` and `CalmDownTransitionDurationSeconds`, and the `CalmDownModeEnabled` toggle itself.
  - Those three had shipped as config-file-only settings with raw, unclamped setters. Exposing them meant bounding them: 200-8000 Hz (below that the low-pass outputs silence, above it the filter is a no-op), 0.05-1.0 volume (0 is what pause is for; above 1 would make Calm Down *louder* than normal) and 0.1-10s.
  - `Default` restates the shipped 1500 Hz / 0.5 / 1.5s exactly, so picking it is a way back rather than an approximation. Every preset value sits on its slider's tick grid — `IsSnapToTickEnabled` coerces off-grid values, which would have moved a preset's sound the moment the page was opened. Two values were adjusted for this (Distant 450 -> 500 Hz, Whisper 0.12 -> 0.10).
  - Moving any slider sets the preset to `Custom`, guarded by an `_applyingPreset` flag so a preset writing its own three values does not immediately flip itself to Custom.
  - `CalmDownFadeLengthMultiplier` is deliberately not exposed: it is declared and filed in the reset map but read nowhere, so a control for it would do nothing.
- **`ShowCalmDownButton` (Media Controls, default off).** A moon button on the Desktop top panel that toggles Calm Down Mode. Fullscreen has had this on its quick menu since 1.5.0; Desktop only had the settings checkbox, which is the wrong place for something reached for because the music is too much right now.
  - The glyph is `U+EF9E`, read out of the cmap of the `icofont.ttf` Playnite bundles rather than recalled — a wrong code fails silently as a blank box in every user's top panel. Dimmed at 0.3 opacity when off and 1.0 when on, matching how the skip button shows itself inactive.
  - Always yielded from `GetTopPanelItems` with `Visible` carrying the setting, the pattern the visualizer and peak meter use, so it needs no restart. Yielding it conditionally would have meant an item that could not be turned back on mid-session — the same constraint the sidebar items have.
  - Toggling routes through `UpdateSettingsFromMenu` rather than mutating the settings object: turning Calm Down on while the player is SDL2 must swap the backend to NAudio (SDL2 has no post-mixer hook to host `CalmDownProcessor`), and that swap hangs off the diff-based `SettingsChanged` event which only a real `UpdateSettings` raises. A direct write would light the moon with nothing dimming the audio. `UpdateSettingsFromMenu` gained a source parameter so the log says which surface made the change.
  - `UpdateIcons` re-reads `CalmDownModeEnabled` each refresh, so the button stays correct when Calm Down is toggled from the settings dialog, a theme binding or the Fullscreen menu.
  - Off by default: the top panel is shared with themes and other extensions, so the button is offered rather than imposed, and nobody's panel changes on upgrade.
...
- **Playnite's own Fullscreen background music cut out seconds after startup** (Solaris [#137](https://github.com/And360red/Solaris/issues/137)). `Mix_HaltMusic` is process-global — SDL_mixer has one music stream, and Playnite's Fullscreen background music runs on the same library UniPlaySong's `SDL2MusicPlayer` uses. `PrewarmExternalPlayer` loads a warm-up file without playing it, and its cleanup ran `Close()` → `Stop()` → `Mix_HaltMusic()`, stopping Playnite mid-track.
  - Credit where it is due: **Charade28** reported it and kept testing through several wrong leads, and **And360red** (Solaris) reproduced it, ruled out default music, isolated it to the notification-sound prewarm from the `[Jingle] Prewarmed external player` log line, and bisected it to 1.7.2. The bisect independently confirmed the cause — `45d857f`, which added the prewarm, is a 1.7.2 commit.
  - Only visible with `SuppressPlayniteBackgroundMusic` **off** — the configuration of someone who wants Playnite's own music and UniPlaySong purely for jingles. With the default suppression on, that music was not playing to begin with, which is why it went unreported for four releases. The reporter bisected it to 1.7.2; `45d857f`, which added the prewarm, is a 1.7.2 commit.
  - `Stop()` now halts only when this instance is using the stream (`_isActive || _isPausedMidPlayback`; `_isActive` is set solely by `Play()`). Same per-instance gate the process-global `Mix_HookMusicFinished` already uses (issue #89). Deliberate suppression is a separate explicit `Mix_HaltMusic` call in `UniPlaySong.cs` and is untouched.
- **The settings Preview buttons played at Music Volume, not Jingle Volume.** `PlayPreview` set `_previewVolume` from `Settings.MusicVolume` directly. Harmless while jingles were a share of music volume; once 1.8.7 decoupled them the preview and the real sound could differ by any amount. `JingleService.JingleLevel` became `internal` and the preview now reads it, so all three paths — jingle, external notification, and preview — share one rule.
- **Saving settings reverted anything changed outside the settings dialog.** `GetSettings` returns a view model cached for the life of the plugin, whose settings object is the one loaded at startup. Every out-of-dialog write REPLACES the live object rather than mutating it — `UpdateSettingsFromMenu` (Fullscreen quick menu, the new Calm Down button) and theme `{PluginSettings}` bindings all route through `SettingsService.UpdateSettings` — so the view model went on holding the startup copy, and `EndEdit` saved it back over the newer one.
  - `BeginEdit` now adopts `SettingsService.Current`, assigned through the `Settings` property so the `PropertyChanged` subscription moves off the stale object (the leak `CancelEdit` already guards against).
  - Reported as "saving any setting turns Calm Down off" — the audio followed the toggle correctly, because `CalmDownProcessor` reads `SettingsService.Current` fresh on every `Read()`; it was the dialog writing the old value back. Calm Down is simply the one you can hear. Same family as the settings-object swap stale-capture fixed for theme media controls in 1.5.8, in a different place.
...
- **Raising Playnite's Background Volume off 0 left UniPlaySong silent for the whole session, still reporting 0.** `SubscribeToFullscreenVolumeChanges` is what keeps `_playniteFullscreenVolume` live, and it was attempted once, from `OnApplicationStarted`, through `Application.Current.MainWindow?.DataContext`. Playnite raises `OnApplicationStarted` from inside `InitializeView()` — called at the end of `OpenView()`, while `CurrentNative.MainWindow` is assigned only *after* `OpenView()` returns — so a null there was ordinary, and the old code logged a warning and gave up. The multiplier then stayed frozen at its startup value: the user raised the slider, Playnite stored it correctly, and UniPlaySong went on multiplying by the zero it had photographed. Reported as "UniPlaySong keeps setting my volume back to 0". Now retried by the startup verify loop, from the window it is handed, in Desktop too, and unsubscribed on dispose (it was leaking onto Playnite's settings object).
- **Starting Playnite minimised or in the tray played music anyway.** The window state was sampled once, in `OnApplicationStarted`; a window that reaches that state afterwards — or is *born* hidden — raises no `StateChanged`/`IsVisibleChanged` to correct it, so no pause source was ever added. Re-minimising by hand worked because that is a real transition. Now polled by a startup verify timer (attachment retried up to ~30s, resync ~10s) that also reads the window Win32 is actually showing: `IsIconic`/`IsWindowVisible`, since a process the shell starts minimised (a "Run: minimized" startup shortcut) is iconic before WPF has an opinion and `WindowState` can still read `Normal`. `CompleteActivationResume` now also drops `Minimized` and `SystemTray` — a confirmed-foreground window is neither, and a source added from a state WPF disagrees with has no event coming to release it.
- **A window-state pause source added by the new startup poll could be left behind with nothing to release it.** WPF raises `StateChanged` and `IsVisibleChanged` only on a *change*, so a source added from a poll — especially from the Win32 reading, which can disagree with WPF — has no event coming to take it back. Stale, it refused every play including the transport buttons, and only an unrelated settings save cleared it. Reported against beta 1 as "it didn't play when I maximised it either; it'd start after I clicked around and opened the settings", with the log showing `Pause: SystemTray` at +2.4 s and `Resume: SystemTray removed` 55 s later on a settings save. `ResyncWindowStatePauseSources` now takes a `bidirectional` flag that also *releases* a source whose condition no longer holds (logging both readings when it does); the startup poll uses it, so a wrong reading self-corrects on the next 200 ms tick. The root fix is at the point of refusal: `MusicPlaybackService` calls an injected `SetWindowStateVerifier` immediately before letting window-state sources block playback — the service has no window to read, the host does — so a stale pause cannot silently block anything.
- **`_mainWindowHandle` and the main window's `Activated` hook sat behind the same bare `if (Application.Current.MainWindow != null)` with no retry.** When it came up empty the handle stayed `IntPtr.Zero` for the session, so every `GetForegroundWindow() != _mainWindowHandle` test read as "Playnite is not in front" and the focus-loss pause had no way back. Both now attach inside `AttachWindowStateHandlers`, which the verify loop retries.

### Added

- **Playnite's Fullscreen Background Volume now controls Spotify while Live Effects are running.** The effected Spotify stream is added to the OUTPUT mixer, deliberately past the master fader, so that fullscreen theme overlays riding the master to 0 cannot silence it — which also meant the fullscreen multiplier, riding on `_musicPlayer.Volume`, never reached it. `NAudioMusicPlayer.ExternalVolumeMultiplier` is now set unconditionally by `SetVolumeMultiplier` (unconditional because the external input plays while the game player is idle, which is exactly when that method's `IsActive` guard is false) and applied in `TargetExternalVolume`; `ExternalGateProvider.Read` already re-ramps on target change over 0.2s, so drags stay smooth. Fullscreen themes with their own volume/mute controls write that slider, so those controls now drive Spotify — joining the reverb, visualizer and Calm Down Mode already applied to it (`c932223`). Desktop was unaffected throughout: the multiplier is 1.0 there, so Music Volume already governed Spotify.
- **Zero-volume notifications for both volumes that can silence playback** — Playnite's fullscreen Background Volume (`WarnAboutZeroedPlayniteVolume`) and UniPlaySong's own Music Volume (`WarnIfUpsVolumeIsZero`, new). Each fires once per session and is **withdrawn and re-armed** via `WithdrawWarning` the moment its value rises above 0; nothing removed them before, so a user who fixed the setting was still being told it was at 0.
- **Diagnostic: `OnFullscreenSettingsChanged` logs `Environment.StackTrace` on any 1 → 0 transition of `BackgroundVolume`.** `PropertyChanged` is raised from the setter, so the caller is still on the stack. UniPlaySong has no writer for that value — pinned by `UniPlaySongNeverWritesPlaynitesVolumeItself` — so whatever appears in that trace is not it.

### Changed

- **The Desktop-mode Background Volume warning is removed.** It fired at every Desktop startup about a value that is inert in that mode, and pointed at a Playnite settings page that does not exist there — `LOCSettingsMusiVolume` lives only in `Playnite.FullscreenApp/Controls/SettingsSections/Audio.xaml`. It produced a field report from a user hunting for a "Fullscreen" tab in Desktop settings. `InitializeFullscreenSettingsRef()` is still called in Desktop so the change-watch can report a zeroing wherever it happens.
- **Notification text reduced to the condition and the fix.** Both now read as a single statement plus a path, with no explanation of the mechanism.
- **`fullscreenConfig.json` is read with `FileShare.ReadWrite`.** That file is Playnite's, and other extensions write it — PS5 Experience ducks Background Volume through it, and Playnite rewrites it on every exit. A plain `File.ReadAllText` holds `FileShare.Read` for the duration of the read, which would make a concurrent write fail with a sharing violation. UniPlaySong reads it only to resolve the active theme id and has no business risking that.
...
- **First game after a cold start played no music, and sometimes the second one too.** `MusicFader.Switch` set the play, stop and preload actions but never cleared `_pauseAction`. `Tick` picks its branch with `_pauseAction == null && _playAction != null` for a switch, so a pause left armed from an earlier startup overlay forced a genuine song change down the PAUSE branch — which cleared only `_pauseAction` and `_stopAction` and **leaked `_playAction`**: the next track was preloaded and then silently never played. The third game onward worked because `_pauseAction` was null by then, which is exactly the reported "first one silent, second one silent, third fine" shape. `Switch` now supersedes a pending pause when it carries a play action, and the PAUSE branch discards any play action queued behind it rather than orphaning it. Diagnosed from a Mike Aniki theme report; the `_pauseAction` half was the reporter's own reading, and holds.
- **Playnite's fullscreen Background Volume at 0.00 silenced UniPlaySong with no explanation.** UPS scales its volume by that slider — which governs *Playnite's* background music, the music UPS suppresses and replaces by default — so zeroing it is the natural move for someone who wants UPS's music instead, and it muted UPS entirely. Every symptom pointed at UPS: its own Music Volume still read 50%, tracks loaded and "played", and the log was clean. The value is still honoured; UPS now raises a one-time notification naming the exact setting, and it is the first entry in the About tab's Troubleshooting card. Confirmed as the cause of a field report of "the music starts muted every time".
- **An unconfigured default music source produced silence instead of falling back.** `QuickStartProfiles.DefaultSourceIsUsable` was consulted only when *applying* a Quick Start profile — changing the source afterwards in settings went unchecked, so `CustomFolder` with no folder ever chosen simply played nothing. Now checked at playback time, substituting the bundled preset and telling the user which setting is unfinished. Deliberately tests only whether the user supplied a path or selection, never whether that path currently exists: stat-ing it would fire on a sleeping NAS or an unplugged drive and accuse a correct setup of being broken.
- **Raising Music Volume back above 0 did not resume playback until the next game switch.** `MusicPlaybackCoordinator.ShouldPlayMusic` refuses at `MusicVolume <= 0`, so zero is a playback *gate*, not a level — but `OnSettingsServiceChanged` only treated `MusicState` and `EnableMusic` as gate changes. Dropping to 0 let playback stop; raising it again set `_targetVolume` via `SetVolume`, which is a no-op when `_musicPlayer.IsActive` is false. The zero *crossing* now joins the other gate changes, and only the crossing — every volume change triggering a replay would restart the song on each drag of the slider. Guarded on `IsPlaying` so a track that survived the silent spell is made audible rather than restarted.
- **Desktop mode gave no warning that a zeroed Background Volume would silence Fullscreen.** The setting is edited from Desktop settings but only applies in Fullscreen, so the consequence arrives later with nothing connecting it to the change. Desktop now reads it at startup and warns; it still does not apply the multiplier, since desktop playback is not scaled by that slider.
- **Radio Mode with an empty pool returned into total silence.** `StartRadioPlayback` was `void` and `PlayGameMusic` returned unconditionally after calling it, so the documented safety net was unreachable — the Jukebox Quick Start profile deliberately keeps default music on for exactly this case, and control never got that far. It now returns `bool` and the caller falls through to normal playback when the radio has nothing to play.
- **Settings changes did not reach three components that captured the old settings object.** Saving settings swaps the whole object, so `MusicControl`, `MusicControlPauseGamePlayDefault` and `RandomPickerMonitor` kept acting on a stale copy — the same class of fault fixed for theme media controls in v1.5.8, with these three left latent. `OnSettingsServiceChanged` now hands them the new object.
- **`ThemeCompatibleSilentSkip` shipped on while its own UI said "Default: Off".** The backing field said `true`. Now `false`, matching what the settings screen has always claimed.
...
- **The no-fade start path warms up too.** "Initial playback with fade-in" has no outgoing track, so no fade-out window exists for the preload to run in and it called `Load()` cold. This is the path the Spotify radio takes when it yields to a selected game on entering Fullscreen Details view. It now builds the reader on a worker and marshals back to the UI thread for the mixer wiring. A `_playbackGeneration` token — bumped by `PlayGameMusic`, `Stop` and `FadeOutAndStop` — drops a warm-up that lands after the user has navigated on, and the player instance is re-checked so a backend swap mid-warm-up cannot start on the wrong player. With no dispatcher (tests, headless) it runs the original synchronous body.
- **Fade timers no longer outrank rendering and input.** `MusicFader._fadeTimer` (20/sec) and both backends' `_rampTimer` (~60/sec) ran at `DispatcherPriority.Normal` (9), above `Render` (7) and `Input` (5), for the duration of every fade — and a fade fires on every game switch. Now `Input` and `Render` respectively. Safe because both ramps derive progress from wall-clock elapsed time (`DateTime.Now - _rampStartTime`) rather than a tick count: a delayed tick yields a coarser volume step but the same completion time and final volume. `NAudioMusicPlayer` needed no change (per-sample ramping on the audio thread, no dispatcher timer); `MediaElementsMonitor` is deliberately untouched.

### Fixed

- **A backend swap left the outgoing `MusicPlaybackService` alive and running.** The swap captured the old instance only to read `UserHasManuallyStartedThisSession`, then reassigned the field. The old *player* was disposed a few lines above; the old *service* never was, and it has no `Dispose`. It owns four dispatcher timers — `_previewTimer`, `_songEndFadeTimer`, `MusicFader`'s fade timer and `CrossfadeCoordinator`'s 500ms poll — and a started `DispatcherTimer` is held by the Dispatcher's timer list, a strong reference. Any orphan with one still ticking was never collected and kept driving the player disposed during the swap. Not limited to the Live Effects / Visualizer / Crossfade toggles: `HandlePlayerSwitchForFormat` fires the same swap on the GME auto-switch, during ordinary library browsing. New `MusicPlaybackService.Shutdown()` stops all four and is called on the outgoing service at both swap sites.
- **`SongMetadataService` never detached from the service it left.** `ResubscribeToService` attached to the new service but not away from the old one, and subscribed with a lambda (`_ => ClearCurrentSongInfo()`) that cannot be passed to `-=`, so detaching was impossible even in principle. Combined with the orphan above, the old service carried on driving metadata extraction and the work per song change multiplied with every swap. Named handler plus an explicit detach. The detach is deliberately *not* skipped when the service is unchanged — `ResubscribeToService` doubles as the "refresh cached info" entry point and callers rely on that; detach-then-attach is idempotent, so it guards double-subscription anyway.

### Changed

- **Normal-use logging is now gated behind Enable Debug Logging.** With debug off, UPS was still writing ~580 always-on call sites into Playnite's *shared* `extension.log`, which every extension writes to. New `Common/GatedLogger` wraps `ILogger` and drops Trace/Debug/Info/Warn when the gate is off; `Error` always passes. Gating happens at the logger rather than the call sites because only 9 of the 209 `Logger.Info/Debug/Warn` sites live in a class that also holds a `FileLogger` — routing them individually would have meant threading a second logger through roughly forty classes. One line changed per file, 74 sites. `FileLogger.Info` is gated too, so `UniPlaySong.log` keeps WARN and ERROR; `Warn` stays ungated there deliberately, since it is our own low-volume file and support keeps that signal. New `FileLogger.Lifecycle()` carries the version banner past the gate, because a report against an unknown build cannot be acted on. Uses the existing `FileLogger.GlobalDebugEnabled` gate, which reads true while unset, so early-startup logging is unchanged. Net with debug off: `extension.log` 580 → 372 sites (errors only), `UniPlaySong.log` 256 → 107 (warn + error).
- **Two regions lifted out of `UniPlaySong.cs`** (6,598 → 6,242 lines), chosen by measuring how many plugin fields each reaches into rather than by size alone. Music Migration (235 lines, 3 methods, touched only `_api`) → `Services/MigrationProgressRunner`; its migration and delete flows were near-identical copies differing only in result mapping and summary wording, and now share one implementation. Default Song Pool (155 lines, 2 methods) → `Services/SongPoolProvider`; its two switch statements repeated the same four gathering strategies with one-word differences, which is how `CompletionStatusPool` gets added to one pool and forgotten in the other. `SongPoolProvider` resolves `_fileService` through a lambda rather than capturing it, because the plugin rebuilds that service when the music path changes. The plugin keeps thin delegating wrappers: the migration methods are public because `UniPlaySongSettingsViewModel` calls them through the plugin, and the pool methods are registered as delegates at three separate points. Six new tests cover the folder-backed pools, the v1.5.8 radio-folder fallback, and the both-pools invariant — 141 tests total.
- One user-visible string: the delete cancellation dialog now reads "Delete was cancelled." rather than "Delete operation was cancelled."

### Added — Hidden achievement tier

- **A sixth achievement rarity, Hidden, for secret unlocks**, sitting between Ultra-Rare and Capstone. Fired by Playnite Achievements as `playnite://uniplaysong/playniteachievements/hidden` — note the segment is `hidden`, not `hiddenachievement`, because that is the command name PA's developer chose; the other five keep the `{rarity}achievement` pattern. Themes supply it as `audio/Achievements/hidden.[ext]` — any of `.mp3`, `.ogg`, `.wav`, `.flac`, first match wins — and the theme status line counts N/6 rather than N/5. The bundled default is `PAStarterPack/hidden.mp3`; the bundled pack is MP3 throughout for size, while theme authors keep the full choice of format. `src/Services/JingleService.cs`, `src/Services/ExternalControlService.cs`, `src/Common/PlayniteThemeHelper.cs`, `src/Services/BundledJingleService.cs`, `src/UniPlaySongSettings.cs`, `src/UniPlaySongSettingsViewModel.cs`, `src/UniPlaySongSettingsView.xaml`, `src/UniPlaySongSettingsView.xaml.cs`.
- **A badge for the new tier** (`src/Images/Achievements/badge-hidden.png`), rendered to match the existing set: flat geometric shape, diagonal metallic gradient, transparent background, 128x128. A star with the question mark knocked out of it, so it reads as "secret" without reusing another tier's artwork. The five existing badges are Justin Delano's PA artwork; this one is generated rather than taken from that set.
- Inserting `AchievementHidden` mid-enum is safe here — `JingleEvent` is runtime dispatch only and never serialized, unlike the settings enums Playnite persists as integers.

### Changed — About tab

- **Retired two stale known issues.** The Windows 11 toast-blur entry (added v1.5.0) described a break that no longer reproduces — blur works again — so it was warning users about nothing. The autosuspend quirk ran four sentences of audio-session detail and promised a fix in "a future v1.5.x patch", two minor versions stale; it is now two lines, why it happens and exit Playnite when you step away. The v1.5.0 Known Issues entry is left in place as a record of what was true then.
- **Active Theme Music explains the requirement, not its history.** The entry walked through the v1.5.2 `background.mp3` → `UPS_BackgroundAudio` rename, why Playnite's own player forced it, and the helper button. Now: the theme needs a `UPS_BackgroundAudio` file, so ask its developer to add one if you want the option.
- **Dropped "theme developers" framing from the Theme Support tab intro** and the supported-themes footer. Addressing developers in a user-facing settings tab implied the options were not for the person reading them; the "do not change these unless a theme tells you to" guidance carries the useful part.
- **Removed Solaris from Officially Supported Fullscreen Themes.**
- **Added a Thanks box** — Mike Aniki for the ANIKI REMAKE integration and the theme-integration features it drove, Justin Delano for the PlayniteAchievements integration, and bug reporters. Kept separate from Bundled Music Credits, which is track licensing.
- **What's New now lists 1.7.1.** The box is hardcoded rather than read from the manifest, so it still showed v1.7.0 items. Five entries against the manifest's eighteen: the fourteen settings-tab entries are one story to a user, so they collapse to a single line and the manifest keeps the per-tab detail.
...
- **`RadioPlaysThroughGames` is theme-bindable** via `{PluginSettings Plugin=UniPlaySong, Path=RadioPlaysThroughGames, Mode=TwoWay}`. No control class or `AddCustomElementSupport` entry needed — `AddSettingsSupport` already exposes every public settings property. Documented in `docs/dev_docs/Theme%20Support/THEME_INTEGRATION_GUIDE.md`.

### Fixed

- **"Play Only on Game Select" did nothing while Radio Mode was on.** Both radio branches in `PlayGameMusic` return early — Spotify radio suppresses UPS and returns, pool radio starts/continues the pool and returns — and both sit *above* the `PlayOnlyOnGameSelect` gate in the normal-playback path, so the setting was structurally unreachable whenever radio was enabled, for either radio type. Radio is the browsing/ambient layer, exactly the role default music plays when radio is off, so the intended shape is: List view → radio, Details view → the selected game's music, back to List → radio resumes. A new pure `RadioGameSelectPolicy` (mirroring `RadioPlayThroughPolicy`) decides the yield and both branches consult it before returning; pool radio clears `_isInRadioMode` so backing out to List restarts it. For Spotify, an explicit `IsRadioYieldedToGameMusic` flag feeds `SpotifyControlService`'s `lifecyclePaused`, so Spotify ducks out and — because UPS owns that pause — `SpotifyRadioDecision` resumes it on List return; that reuses the existing seam instead of injecting Spotify into `MusicPlaybackService`, the dependency inversion behind the old launch-freeze. The flag is deliberately *not* inferred from `IsPlaying`: by the `IMusicPlayer.IsActive` invariant that counts a paused-mid-playback song as active, so a stale loaded song would have held Spotify paused indefinitely. `src/Services/RadioGameSelectPolicy.cs` (new), `src/Services/MusicPlaybackService.cs`, `src/Services/IMusicPlaybackService.cs`, `src/Services/Spotify/SpotifyControlService.cs`.
- **The same song replayed on every Details-view entry under radio + "Play Only on Game Select".** `forceRandomizeOnDetailsEntry` was gated on `_isPlayingDefaultMusic`, which is false when *radio* rather than default music is the browsing layer, so randomization never armed and `SelectSongToPlay` fell through to its alphabetical `FirstOrDefault()` branch. The radio yield now arms it too; `RandomizeOnEverySelect` still gates the actual pick. `src/Services/MusicPlaybackService.cs`.
- **Taskbar thumbnail buttons and media keys controlled UPS instead of Spotify.** Both share the `OnMediaKey*` handlers, which drove `_playbackService` directly and predate `ActiveMediaService`. With Spotify as the radio source UPS's own player is suppressed but still loaded, so pressing play started UPS game music *alongside* Spotify and next/previous skipped UPS songs rather than Spotify tracks. A narrow Spotify branch now routes through `ActiveMediaService`, keyed on the same `IsSpotifyActive` signal `ResolveSource()` uses; when Spotify is not active the handlers fall through to the original UPS logic unchanged, guard included. (Routing *everything* through `ActiveMediaService.PlayPause()` silently drops the `IsPlaying || IsLoaded` guard, which pins a sticky `Manual` pause when pressed with nothing loaded — hence the narrow branch.) `src/UniPlaySong.cs`.
- **The taskbar play/pause icon showed the wrong state, and lagged, while Spotify was the source.** It read UPS's player, so it sat on "Play" during audible Spotify playback; and it was subscribed only to UPS playback events, which Spotify's play/pause never raises, so it refreshed only when some unrelated UPS event happened to fire. It now reports whichever source is active (Spotify via the client's cached, non-blocking `IsSpotifyPlaying`), including on first attach, and the taskbar subscribes to `SpotifyControlService.NowPlayingChanged`. Marshalled with `BeginInvoke` — `ThumbButtonInfo.ImageSource` requires the UI thread, and a synchronous marshal from an SMTC callback thread can deadlock against a UI thread inside `Recompute` — and unsubscribed in teardown. `src/UniPlaySong.cs`.
- **Live Effects on Spotify produced static/dropouts when the Fullscreen theme stalled.** With effects on you no longer hear Spotify — it is ducked to 2⁻¹⁰ and the audible output is UPS's own NAudio pipeline replaying the captured stream, so the effected output is only as smooth as UPS's audio path inside Playnite's process. The native WASAPI capture buffer was the binding constraint at 200 ms: the capture loop hands PCM to a *managed* callback, so any managed stall (a gen2 GC pause, a heavy theme view load) blocks that thread, WASAPI overflows, and the overflowed audio is discarded at the source where no downstream buffer can recover it. Raised to 5 s, matching OBS win-capture-audio — capacity, not latency, since packets are still delivered as they arrive — and the managed ring went 250 ms → ~1 s so it can accept the catch-up burst when a stalled callback unblocks. `native/SpotifyLoopback/SpotifyLoopbackCapture.cpp`, `src/Common/SpotifyLoopbackClient.cs`, rebuilt `src/Audio/Native/SpotifyLoopback.dll` (x86, exports verified).
...
- **Playnite could take seconds to shut down, worsening the longer the session ran.** Two compounding costs on the *unconditional* shutdown path (`OnApplicationStopped` → `RestoreSpotifyForExit`), both of which hit users who never touch Spotify. First, the restore loop could never confirm without a Spotify session — `GetSessionVolume` returns its `0f` fallback when none is found, making `now >= restore * 0.9f` unsatisfiable — so every shutdown burned all 12 attempts: 300 ms of sleeps plus ~70 full multi-device COM enumerations. `SetMuted` already reports whether any session was found, so it now bails immediately when there is nothing to restore. Second, `VisitSpotifySessions` called `Process.GetProcessById` for every audio session on every active render device purely to filter by name: that opens a process handle per session and never disposes it (a leaked handle per session per enumeration) and throws for the dead PIDs of expired sessions, which accumulate the longer Spotify runs — a plausible mechanism for the "slower the longer it's been open" reports, and a tax on every duck, read and cache refresh at runtime too. Spotify's PIDs are now resolved once per enumeration into a `HashSet` (those `Process` objects disposed) and matched by integer, with an early return when Spotify isn't running at all. `src/Common/SpotifyAudioSession.cs`, `src/Services/Spotify/SpotifyLiveEffectsHost.cs`.
- **Choppy Spotify audio on game switches with Live Effects on.** v1.6.9 made `SetSessionVolume` enumerate every active render device × every Spotify session over COM (~90 ms on an active session) and added a duck re-assert on every coordinator `Evaluate`. Those compound: while Spotify radio is active `RecomputeCore` raises `NowPlayingChanged` on *every* `Recompute`, which drives `Evaluate`, so a game switch fired a burst of re-asserts — each costing two full enumerations (the `GetSessionVolume` read plus the `SetSessionVolume` write). The first duck still applies immediately; redundant re-asserts are now capped at one per 5 s, so an output-device switch spawning a fresh unducked session still self-heals within an interval instead of re-ducking on every event. `src/Services/Spotify/SpotifyLiveEffectsHost.cs`.
...
- **Game music restarted from 0:00 (or wedged paused) around focus changes.** With "Pause on focus loss" enabled, returning focus to Playnite restarted game music from the beginning instead of resuming — or playback stayed paused with the top-panel play button dead. Three stacked defects: (1) SDL2's `Pause()` dropped `IsActive`, so `RemovePauseSource`'s resume logic couldn't tell a paused song from a loaded-but-never-started one and took the `Play()` (restart) branch — NAudio was immune because `_logicallyPaused` counts as active, which is why Spotify/effects users never saw it; SDL2 now has NAudio-parity logical pause (`_isPausedMidPlayback`). (2) `MusicFader.FadeIn()` never cleared the fader's `_isPaused`, so `HasPendingPlayAction` went stale-true and fired zombie load+plays on later pause-source removals. (3) A `FocusLoss` source stuck by the sibling-window activation guard (Win32 focus in a WebView2/sibling HWND) could never be cleared by top-panel or URI play, which removed only `Manual` — explicit play now clears `FocusLoss` too. Masked until v1.6.3: the issue-#89 hook-steal bug killed SDL2 music after one track, hiding the broken resume path. Invariant going forward: `IMusicPlayer.IsActive` must include paused-mid-playback on every backend. `src/Services/SDL2MusicPlayer.cs`, `src/Players/MusicFader.cs`, `src/Services/MusicPlaybackService.cs`, `src/Services/ExternalControlService.cs`.
- **Preview mode could restart a song audibly while paused.** The preview timer keeps running through a focus-loss pause (unlike manual pause); its tick now checks the pause state before restarting/randomizing. Was a latent NAudio bug, would have surfaced on SDL2 with the logical-pause change.
- **Live Effects on Spotify could duck the wrong Spotify audio session (unmuted/doubled audio).** `SpotifyAudioSession` enumerated only the DEFAULT render endpoint and acted on the first name-matching session — but Spotify can own several sessions at once (one per output device it has touched, per child PID, plus expired leftovers) and the live one isn't always on the default device (headset/DAC/virtual devices on gaming PCs). The duck could land on a dead session, report success, and effected output would start over unducked raw Spotify. Writes (mute/duck/volume) now apply to EVERY Spotify session on EVERY active render device; reads prefer the actively-rendering session. The coordinator also re-asserts the duck on every evaluate — an output-device switch mid-effects spawns a fresh session at that device's stored volume (unducked) that nothing previously re-ducked; the saved restore level is preserved across re-asserts. New regression test; 115/115 pass. `src/Common/SpotifyAudioSession.cs`, `src/Services/Spotify/SpotifyEffectsCoordinator.cs`, `src/Services/Spotify/SpotifyLiveEffectsHost.cs`.

### Improved

- **Unsupported-Windows notice for Spotify effects.** "Apply Live Effects to Spotify" and Calm Down on Spotify require per-app audio capture — Windows 10 build 20348+, which in practice means Windows 11. On older builds the OS gate silently never engaged: Spotify played dry at full mixer volume while the user believed effects were on (seen in the wild on a Windows 10 19045 gaming PC). UniPlaySong now shows a one-time error notification explaining the limitation when the settings ask for effects-on-Spotify on an incapable OS. `src/Services/Spotify/SpotifyLiveEffectsHost.cs`, `src/UniPlaySong.cs`.

### Fix Attempt

- **Fix attempt (pause on focus loss): music sometimes didn't auto-resume after a long absence.** Intermittent residual of the focus-loss fixes above: manual play always recovered it, proving the stranded pause source was `FocusLoss` — `OnApplicationActivate` never cleared it. Two mechanisms: (1) `Application.Activated` can fire while `GetForegroundWindow()` still reports the previous app's HWND for a few ms (foreground-transfer race, worse after long absences), tripping the v1.5.3 sibling-window guard; (2) the guard's bail was permanent — `Application.Activated` never re-fires for within-app focus returns, so activation landing on a Playnite-owned sibling window stranded `FocusLoss` until a clean unfocus/refocus cycle. Fix attempt: the guard now starts a bounded re-verify (100ms × 20) that clears the pause once the main window is genuinely foreground (giving up after ~2s so the Keyboard Launcher stays paused — issue #79 preserved), the main window's own `Activated` event is also wired to the handler (it does fire on within-app returns), and a genuine deactivation cancels any pending re-verify. `src/UniPlaySong.cs`.
- **Fix attempt (issue #87): auto-download failures now name the affected games.** The library-update / game-install auto-download summary notification only reported counts ("Failed: 2"), giving users nothing to act on. Failures now list the failed game names (capped at 8, "+N more" beyond that) with a retry hint, and the notification uses the Error type when anything failed so it stands out in Playnite's notification panel instead of blending in as Info. Awaiting reporter confirmation. `src/UniPlaySong.cs` (`AutoDownloadMusicForGamesAsync`).
...
> Detailed release notes (test scenarios, full bug context): [v1.4.6-beta release](https://github.com/aHuddini/UniPlaySong/releases/tag/1.4.6-beta). Theme integration deep-dive in [THEME_INTEGRATION_GUIDE.md](docs/dev_docs/Theme%20Support/THEME_INTEGRATION_GUIDE.md).
...
- **LGPL §6 compliance for bundled GME** — added `NOTICES.txt`, `docs/dev_docs/features/CHIPTUNE_GME_DLL_BUILD.md` (pinned commit + reproducible build), and committed the GME source tarball at `lib/source/gme-source-1815b97.tar.gz`. Bundled `gme.dll` is unmodified upstream at commit `1815b97`; UPS does not maintain a fork.
