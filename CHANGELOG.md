...
### Documentation
- **Celebration Trigger String-Match Tradeoff Documented** — Added entry to `POTENTIAL_ISSUES.md` noting that `UniPlaySong.OnItemUpdated` matches completion-status *Names* ("Completed", "Beaten") as string literals rather than by `CompletionStatusId` GUID. This silently breaks the fanfare for users who rename built-in statuses or run Playnite in a non-English locale. Deferred for v1.5 (the fix requires settings schema migration from two bools to a `Guid[]` array, plus a one-time name→ID resolution at first load). Queued alongside the broader idea of letting users pick any subset of statuses to celebrate (mirroring the `CompletionStatusSelectionDialog` pattern used by Nostalgia Mode).
- **About Tab: Supported Formats Section** — Added a "Supported Audio Formats" box to the About settings tab listing the four verified formats (MP3, OGG Vorbis, FLAC, WAV) plus VGM/VGZ with its explicit system scope (Sega Genesis / Mega Drive / Sega CD). Sets realistic user expectations for what plays end-to-end on the default SDL2 backend.
