...
- **`Enable Music` toggle in Fullscreen Extensions menu** (Menu → Extensions → UniPlaySong) — joins the existing Radio Mode / Live Effects / Preview Mode / Play Only on Game Select / Music Only for Installed Games toggles so users on themes without custom audio bindings can still flip the master switch from a controller. Listed first as the most-prominent toggle.
