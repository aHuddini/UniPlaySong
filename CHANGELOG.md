...
- **Spotify Radio Mode intermittent launch freeze (fix attempt) + slow radio start in Fullscreen.** Root cause of the freeze: the radio path in `SpotifyControlService.Recompute` raised `NowPlayingChanged` while still holding `_recomputeLock`; subscribers marshal to the UI thread with a synchronous `Dispatcher.Invoke`, and when the UI thread was simultaneously entering `Recompute` (pause-source churn during launch/game-select), the two waited on each other — a lock-vs-dispatcher deadlock, intermittent because it needs an SMTC/WinRT event racing a UI-thread recompute (widest window on a fresh launch; only the radio path raised inside the lock, which is why no other playback option froze). Fixes: the raise is hoisted outside the lock (deterministic regression test added); all three now-playing UI marshals (`NowPlayingPanel`, `NowPlayingMiniPlayerModel`, settings-view preview) are async `BeginInvoke` now, removing the whole deadlock class; SMTC/WinRT startup (`MediaManager.Start` + first cache read) moved off the plugin-constructor/UI thread onto the Spotify worker, so a cold WinRT broker can no longer stall Playnite's launch; and the availability poll tightened 7s→3s with an immediate `AvailabilityChanged` announce once SMTC startup completes — Spotify radio engages seconds sooner after the Fullscreen login. `src/Services/Spotify/SpotifyControlService.cs`, `src/Services/Spotify/SpotifySmtcClient.cs`, `src/DeskMediaControl/NowPlayingPanel.cs`, `src/Controls/NowPlayingMiniPlayerModel.cs`, `src/UniPlaySongSettingsView.xaml.cs`.

- **Theme "Radio Mode" toggle ON left game music playing alongside Spotify.** The toggle-ON handler only called `StartRadioPlayback`, whose Spotify guard correctly no-ops — but nothing stopped the already-playing game music. It now routes through `PlayGameMusic`'s SpotifyRadioMode suppression branch (fade-out + suppress) and recomputes immediately so Spotify engages without waiting for the poll. Also: the radio engage-Resume is now gated on `OnApplicationStarted` — it previously fired in the startup window before the overlay/login pause sources registered, causing the pause→play→pause churn heard while switching Desktop→Fullscreen. `src/UniPlaySong.cs`, `src/Services/Spotify/SpotifyControlService.cs`.
...
- **Spotify Radio Mode no longer freezes Playnite (rebuild).** All Spotify SMTC calls now run on a dedicated background worker thread instead of the UI thread — the launch / Desktop↔Fullscreen freeze (which only happened with Spotify Radio Mode on) is eliminated. Radio control was rebuilt as a minimal state machine that pauses Spotify on game launch/video/lock and resumes after, respects a pause you make in the Spotify app (it won't auto-resume until you resume), and updates now-playing off-thread. `src/Services/Spotify/SpotifySmtcWorker.cs`, `src/Services/Spotify/SpotifySmtcClient.cs`, `src/Services/Spotify/SpotifyControlService.cs`, `src/Services/Spotify/SpotifyRadioDecision.cs`.

- **Radio Mode unified with Spotify (fixes theme conflict).** Spotify is now a Radio Mode *source* (Playback → Radio Mode → source), alongside the library/folder/rotation/completion pools — replacing the separate "Spotify Radio Mode" toggle. A theme's "Radio Mode" button (e.g. ANIKI ReMake) now plays whichever source you picked, so it no longer forces UPS's own radio over Spotify or plays both at once. Also fixed: turning Spotify radio off (or switching its source) now pauses Spotify instead of leaving it playing. Existing Spotify-radio users: re-select "Spotify" as your Radio Mode source after updating. `src/UniPlaySongSettings.cs`, `src/Services/Spotify/SpotifyControlService.cs`, `src/Services/MusicPlaybackService.cs`.
