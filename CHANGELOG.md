...
### Added
- **Theme integration via Playnite's `{PluginSettings}` markup extension.** UPS now calls `AddSettingsSupport(SourceName="UniPlaySong", SettingsRoot="Settings")` and exposes a public `Settings` property on the plugin class, so themes can bind directly to `UniPlaySongSettings` properties from XAML without needing a custom UPS element in their visual tree:
  ```xml
  <CheckBox IsChecked="{PluginSettings Plugin=UniPlaySong, Path=EnableMusic, Mode=TwoWay}"/>
  ```
  Recommended bindable settings: `EnableMusic`, `EnableDefaultMusic`, `RadioModeEnabled`, `PlayOnlyOnGameSelect`. Two-way sync with the desktop settings dialog via `INotifyPropertyChanged`. Replaces an earlier in-development approach (reverted in commit `3092329`) that added bindable properties to the `UPS_MusicControl` element directly — that pattern crashed themes with `Cannot create unknown type` because WPF resolves theme XAML namespaces before plugin DLLs are loaded. `{PluginSettings}` defers resolution until `ExtensionsLoaded` fires and gracefully no-ops when UPS isn't installed. Same pattern Mike Aniki's own `AnikiHelper` plugin uses. The legacy `UPS_MusicControl` element remains supported and unchanged for pause/resume bindings driven by theme overlay state. Full reference: [`docs/dev_docs/THEME_INTEGRATION_GUIDE.md`](docs/dev_docs/THEME_INTEGRATION_GUIDE.md).
- **`Enable Game Music` and `Enable Default Music` toggles in Fullscreen Extensions menu** (Menu → Extensions → UniPlaySong) — paired at the top of the menu so users on themes without custom audio bindings can flip both layers from a controller. Game Music maps to `EnableMusic` (per-game music folders); Default Music maps to `EnableDefaultMusic` (Bundled Ambient / fallback ambient). Toggling only Game Music off leaves ambient running; toggle both off for full silence. Joins the existing Radio Mode / Live Effects / Preview Mode / Play Only on Game Select / Music Only for Installed Games toggles. Note: the menu label is `Enable Game Music` for clarity, but the underlying setting and theme-binding path remain `EnableMusic` (no migration needed).
- **Two new Bundled Ambient tracks from Mike Aniki** (composer of the [Aniki ReMake](https://github.com/Mike-Aniki/Aniki-ReMake) theme) — `ANIKI REMAKE - Hub OST.mp3` and `ANIKI REMAKE - Login OST.mp3` ship in the `.pext`. Pick from **Settings → Playback → Default Music Source → Bundled Ambient**. Files are gitignored (ship from local copy via packaging script). Attribution in `NOTICES.txt`.
- **HES (NEC TurboGrafx-16 / PC Engine) chiptune support — verified working.** Two playback paths, both requiring a sibling `.m3u` sidecar in GME's extended format (the standard convention used by Zophar's Domain and VGMRips packs). The HES format itself stores no track count, so the M3U is the only way to enumerate tracks — without it, only the file's default `first_track` byte plays and the rest of the songs are invisible.
  ...
  - **"Split HES Tracks" menu action** (right-click game → Chiptunes → Split HES Tracks): splits a multi-track `.hes` into N mini-HES files, one per M3U-listed track, by patching `first_track` (header byte 5) on each copy. After splitting, UPS sees N independent `.hes` files in the music folder, so skip / shuffle / pause / random-pick all work track-by-track via existing playback paths — no `MusicPlaybackService` changes needed. Originals (`.hes` + `.m3u`) preserved to `PreservedOriginals/<GameId>/`. Menu item only appears when the music folder contains a splittable HES (sidecar present, ≥2 tracks). New files: `src/Audio/HesHeaderPatcher.cs`, `src/Handlers/HesSplitHandler.cs`. Mirrors the NSF Manager's split-tracks pattern. `GmeReader` extended to honor HES `first_track` byte 5 on single-track mini files, parallel to its existing NSF `starting_song` byte 7 handling.
