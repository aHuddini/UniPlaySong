...
- **Session Auto-Play Lock Re-Engaged After Backend Recreation** — Toggling `LiveEffectsEnabled` / `ShowSpectrumVisualizer` / `EnableTrueCrossfade` triggers `RecreateMusicPlayerForLiveEffects` which spins up a fresh `MusicPlaybackService`. The new service's `_userHasManuallyStartedThisSession` defaulted to `false`, re-engaging the Desktop auto-play lock even after the user had already manually started playback earlier in the session — subsequent game selections silently refused to play. Fix in `UniPlaySong.cs`: snapshot the old service's `UserHasManuallyStartedThisSession`, then call `NotifyManualStart()` on the new service when the flag was set. Applies to all three `CreateMusicPlayer` recreation sites.
