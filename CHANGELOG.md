...
- **Theme integration — fullscreen-bindable settings.** `UPS_MusicControl` now exposes three of the most-toggled UPS settings as two-way bindable WPF properties so Fullscreen theme authors can wire CheckBox / ToggleButton state directly to UPS settings: `EnableMusic`, `RadioModeEnabled`, `PlayOnlyOnGameSelect`. Standard pattern: place a `<UPS:MusicControl x:Name="upsAudio" Visibility="Collapsed"/>` somewhere in the theme's XAML and bind via `IsChecked="{Binding ElementName=upsAudio, Path=EnableMusic, Mode=TwoWay}"`. Setter assignments flow through the existing `UniPlaySongSettings` PropertyChanged → settings-service → playback-coordinator pipeline, so a theme toggle has the same effect as flipping the corresponding setting in the desktop UPS settings dialog. Designed in collaboration with Mike Aniki for the Aniki ReMake theme's quick-audio-settings menu.
